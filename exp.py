...
from API.typhhon_loader import TLoader 
from API.track_loader import TTLoader
...
class Exp:
    ...
    def _build_model(self):
        ...
        num_layers = 1
        heads = 4
        self.model = TrajectoryTransformer(input_seq_len, output_seq_len, video_dim, tensor_dim, embed_size, num_layers, heads).to(device)
        #self.model = IAM4VP([[12,9,240,320]]).to(device)
        #self.model = IAM4VP(tuple(args.in_shape), args.hid_S, args.hid_T, args.N_S, args.N_T).to(self.device)
    ...
    def _get_data(self):
        ...
        train_dataset = TTLoader()
        self.train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=32, shuffle=True, pin_memory=True, num_workers=8)
    ...
    def train(self, args):
        ...
        for epoch in range(config['epochs']):
            ...
            for _, batch_x, era5, batch_y in train_pbar:
                self.optimizer.zero_grad()
                #batch_x, batch_y = batch_x.to(self.device), batch_y.to(self.device)
                # obs, coords1, future, coords2
                batch_x, era5, batch_y =  batch_x.to(self.device), era5.to(self.device), batch_y.to(self.device)
                #pred_y = self.model(batch_x, era5)
                pred_y = self.model(batch_x, era5)
